...
**Structural-provenance rule invariant (#2576):** A tree-sitter security rule may ADD suppressions on same-file AST provenance, but never trade an existing one for it — narrowing a shipped exemption regresses every user who relies on it (#2577 review). The Python SQL rule therefore keeps both name-shaped exemptions (`session.execute(...)` by receiver name; a `select/insert/update/delete(...)` builder in argument position) and layers provenance on top: when an unshadowed `sqlalchemy.orm.Session` / `sqlalchemy.ext.asyncio.AsyncSession` import and a typed receiver prove the relationship, `query` is suppressed outright and `execute` is suppressed for a builder call or a name bound exactly once to one (`stmt = select(User)`). The sink set stays `execute`/`executemany`/`query`/`raw`: `scalar`/`scalars` take a statement object and adding them only manufactured findings on `client.scalar(stmt)`. `text(...)` counts only with a literal template, so composed SQL stays diagnostic. psycopg composition is suppressed only for an unshadowed `psycopg`/`psycopg2` `sql` import, a static `sql.SQL` template, and `sql.Identifier`-only format arguments. Missing, duplicate, shadowed, or reassigned provenance remains diagnostic.
