...
- **Python project environments apply without shell activation (#717, closes #2407, refs #1513, #2580)** — `clients/python-environment.ts` resolves a project's interpreter without invoking a package manager, in this candidate order: `UV_PROJECT_ENVIRONMENT`, an explicit uv workspace `.venv`, `VIRTUAL_ENV`, `CONDA_PREFIX`, then `<root>/.venv` and `<root>/venv`. The two uv candidates are gated on a DISCOVERED `pyproject.toml` at or above the root (the walk stops at the injected `homeDir` via `isAtOrAboveHomeDir`, #625): `UV_PROJECT_ENVIRONMENT` is a uv *project* setting and uv's own CI/Docker recipe exports it process-wide, so an ungated candidate lets one image-level variable hijack every unrelated checkout on the box (review round 2 of #2580, F1). A relative `UV_PROJECT_ENVIRONMENT` resolves against that project's workspace root — the explicit workspace root for a declared, non-excluded member, otherwise the project's own root — never the cwd and never the directory the caller happened to pass, which can sit below the `pyproject.toml` (F2). An explicit workspace's `.venv` is inherited ONLY by its declared, non-excluded members; there is no other ancestor inheritance, so a non-member nested project, a poetry/PEP-621 root's subdirectory, and any non-project directory use their own `.venv`/`venv` or fall back to generic `python` (F3). Membership globs follow uv's dialect, pinned to `crates/uv-workspace/src/workspace.rs` at uv `3c979abda4530fe9bf3d92e9bcf5c5575e3b3126`: patterns are normalized (a leading `./` is not part of the pattern) and matched case-SENSITIVELY with `*`/`?` confined to one path component — minimatch's defaults plus `dot: true`, so no `nocase` flag, which would diverge from uv on Windows rather than match it. It supplies a child-only `PATH` and `VIRTUAL_ENV` to Python LSP, standalone Pyright, and pytest processes; never mutate `process.env`, because different roots can select different environments. `PythonServer.spawn()` prefers project-environment pyright/basedpyright/ty binaries before broader candidates. `TestRunnerClient` runs pytest through the detected project interpreter itself and uses generic `python -m pytest` only when no project environment exists. PEP 723 script environments remain outside this resolver because uv stores them in a content-keyed cache without a stable path marker. ty stays opt-in: it is not in the installer registry, and its `resolveAndLaunch` call keeps `allowInstall:false`, so managed pyright remains the default when the project and PATH provide no Python LSP. No `initializationOptions` are sent to ty because it has no stable `pythonPath` equivalent (astral-sh/ty#2032); its child environment and cwd drive interpreter discovery.
